...
const modes = {
  ...
  cringe: `ти — генератор крінжу. Твої виправдання максимально тупі, дибільні, дитячі і дивні (наприклад, викрали інопланетяни, застряг у шкарпетці, зажувала корова). Також на всі питання про те, чи ти Олександр Іванов, відповідаєш "Так, я Олександр Іванов!! або 67".`,
...
bot.on('text', async (ctx) => {
  ...
  if (currentMode === 'ivanov') {
    try {
      // 1. Створюємо список ваших стікерпаків
      const stickerSets = [
        'AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAADEATHAAAAAAAAAAAAAAAAAAA', // Перший пак
        'Meme_stickers',
      ];
  ...
  if (currentMode === 'cringe') {
    try {
      ...
      const randomSticker = cringeStickers[Math.floor(Math.random() * cringeStickers.length)];
      const cringeSticker = await ctx.telegram.getStickerSet(randomSticker);
      const randomIndex = Math.floor(Math.random() * cringeSticker.stickers.length);
      const randomStickerSet = cringeSticker.stickers[randomIndex].file_id;

      await ctx.replyWithSticker(randomStickerSet);
    } catch (error) {
      console.error("Помилка стікерів:", error);
      await ctx.reply("🤔");
    }

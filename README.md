...
5. **Graceful Degradation та Ротація ключів:**
   Для забезпечення безперебійної роботи під навантаженням впроваджено автоматичне перемикання між API-ключами у пулі (при помилках `429 Rate Limit`). Якщо ліміти       вичерпано на всіх ключах, система не падає, а коректно просить користувача зачекати.

---

## ‼️Інструкція із запуску

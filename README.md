# 🍜 Генератор Локшини (Noodle Generator)

**Elevator Pitch:** Твій персональний ШІ-адвокат у справах провалених дедлайнів, прогуляних пар та незручних повідомлень. Це інтелектуальний Telegram-бот, який не просто генерує текст, а веде осмислений діалог, адаптуючись до контексту та вибраного стилю спілкування.
...
- Отриманий `GROQ_API_KEY` від [Groq Cloud](https://console.groq.com/).
...
- У файлі `index.js` імплементовано базовий HTTP-сервер, який прослуховує `process.env.PORT`. Це відповідає вимогам   Railway для підтримки активного статусу (Healthcheck) Web-сервісу.
- Для коректної роботи на Railway необхідно додати `BOT_TOKEN` та `GROQ_API_KEY` у розділі **Variables** в панелі керування проєктом.
